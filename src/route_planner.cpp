...
RouteModel::Node *RoutePlanner::NextNode() {
    std::sort(open_list.begin(), open_list.end(), [](RouteModel::Node *n1, RouteModel::Node *n2) -> bool {
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    for (int i = 0, j = path_found.size() - 1; i < j; i++, j--) {
        RouteModel::Node temp = path_found[i];
        path_found[i] = path_found[j];
        path_found[j] = temp;
    }
...
// TODO 7: [DONE] 4Write the A* Search algorithm here.
// Tips:
